...
object RetailStoreDiscount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val output = transactions.map(transaction => {
      ...
      s"${transaction.timestamp},${transaction.productName},${transaction.expiryDate},${transaction.quantity},${transaction.unitPrice},${transaction.channel},${transaction.paymentMethod},$discount,$totalPriceAfterDiscount"
    })
